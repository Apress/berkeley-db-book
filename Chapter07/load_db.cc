...
DbUtils_SP DbUtils::singleton_;
ACE_Recursive_Thread_Mutex DbUtils::initMutex_;
ACE_thread_t DbUtils::mainThId_;
int DbUtils::processNumber_ = 0;

int DbUtils::isAlive(DbEnv *env,
                     pid_t pid,
                     db_threadid_t tid)
{
    if(mainThId_ == tid || pid != ACE_OS::getpid())
    {
        return 1;
    }
    ...
    ACE_UINT32 state = 0;
    ACE_thread_t id = tid;
    int ret = tm->thr_state(id, state);
    //ACE_DEBUG((LM_ERROR, 
    //            "DbUtils::isAlive: ret %d tid %u state %u\n",
    //            ret, id, state));
    if(ret == false || 
       state == ACE_Thread_Manager::ACE_THR_CANCELLED || 
       state == ACE_Thread_Manager::ACE_THR_TERMINATED)
        return 0;
    else
        return 1;
}

void DbUtils::threadId(DbEnv *env,
                        pid_t *pid,
                        db_threadid_t *tid)
{
    ...
    *pid = ACE_OS::getpid();
    //ACE_DEBUG((LM_ERROR, 
    //            "DbUtils::(%t) threadId: returning %u\n",
    //            id));
}
    ...
    dbEnv_(0)
{
    try
    {
        const char *envhome = "./chap7_env";
        dbEnv_.set_errpfx("chap7_ex");
        ...
        if(processNumber_ == 1)
            envFlags |= DB_INIT_LOG | DB_RECOVER | DB_CREATE;
        dbEnv_.open(envhome, envFlags, 0);
        dbEnv_.set_thread_id(DbUtils::threadId);
        dbEnv_.set_isalive(DbUtils::isAlive);
    }
...
void DbUtils::loadDbs()
{
    ...
    ACE_DEBUG((LM_ERROR, "loadDbs: going to load Dbs processNumber_ %d\n", 
                processNumber_));
    if(processNumber_ != 1)
        return;
...
int DbEntry::dbGet(Db *db, Dbt *key, Dbt* data, DbTxn* t)
{
    ...
    try
    {
        ...
        {
            ACE_DEBUG((LM_ERROR, 
                        "DbEntry::dbGet: dbenv_failchk returned %d\n",
                        state));
            ACE_OS::exit(1);
        }
        if(t == NULL)
...
//#if 0
        ...
        {
            ACE_DEBUG((LM_ERROR, 
                        "DbEntry::dbPut: dbenv_failchk returned %d\n",
                        state));
            ACE_OS::exit(1);
        }
//#endif
