...
int DbUtils::unrelated_ = 1;
int DbUtils::runRecovery_ = 0;
int DbUtils::repId_ = DB_EID_INVALID;
...
DbUtils::DbUtils():
    dbEnv_(0)
{
    initEnv(DB_REP_CLIENT);
}
void DbUtils::initEnv(int role)
{
    try
    {
...
void DbUtils::loadDbs(int role)
{
    ...
    ACE_DEBUG((LM_ERROR, 
                "loadDbs: closed existing handles \n"));

    int dbFlags = DB_THREAD;
    if(role == DB_REP_MASTER)
        dbFlags |= DB_CREATE;
    else if(role == DB_REP_CLIENT)
        dbFlags |= DB_RDONLY;
